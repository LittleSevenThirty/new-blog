      ...
      children: [
        ...
        new Paragraph({
          children: [new TextRun({
            text: "我设计并实现了一个基于Spring Boot和Vue 3的博客系统，采用前后端分离架构，包含前端展示、后端管理和数据存储三个核心模块。前端使用Vue 3 + TypeScript + Element Plus，实现了响应式布局、Markdown编辑和用户交互功能；后端采用Spring Boot 3.4.0 + MyBatis Plus + MySQL，完成了用户管理、文章管理和评论管理等核心功能；数据存储使用MySQL数据库，设计了合理的表结构支撑系统运行。系统还集成了Redis缓存、JWT认证和RabbitMQ消息队列，提升了性能和安全性。本文详细介绍了系统的需求分析、设计方案、实现过程和测试结果，展示了软件工程专业知识在实际项目中的应用。通过这个项目，我不仅巩固了所学知识，还提高了实际开发能力和问题解决能力，为未来职业发展打下了基础。"
          })
        ],
        spacing: { before: 360, after: 360 }
        }),
        ...
        new Paragraph({
          children: [new TextRun({
            text: "随着互联网的快速发展，博客已经成为人们日常生活和工作中重要的信息分享平台。传统博客系统往往功能单一、用户体验差、维护成本高，难以满足现代用户的需求。因此，设计一个功能完善、性能优异、用户体验良好的博客系统具有现实意义。"
          })
        ],
        spacing: { before: 360, after: 360 }
        }),
        ...
        new Paragraph({
          children: [new TextRun({
            text: "在信息爆炸的时代，人们需要高效、便捷的平台分享和获取信息。博客作为个人或组织展示思想、分享知识的工具，已被广泛应用。但传统博客系统在功能、性能和用户体验方面存在不足，比如界面老旧、响应慢、功能单一。随着Web技术的发展，前后端分离架构、微服务等技术为构建现代化博客系统提供了新可能。"
          })
        ],
        spacing: { before: 360, after: 360 }
        }),
        ...
        new Paragraph({
          children: [new TextRun({
            text: "我希望设计并实现一个基于Spring Boot和Vue 3的现代博客系统，通过前后端分离架构和当前流行技术栈，构建一个功能齐全、性能优越、用户体验良好的平台。系统将支持文章管理、用户管理、评论管理、标签管理等核心功能，同时提供友好的后台管理界面，方便管理员维护和管理内容。"
          })
        ],
        spacing: { before: 360, after: 360 }
        }),
        ...
        new Paragraph({
          children: [new TextRun({ text: "本项目的研究内容包括：" })]
        }),
        new Paragraph({
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: "文章展示：支持文章列表、详情、分类浏览、标签浏览" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "用户交互：支持注册、登录、评论、点赞、收藏" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "个性化设置：支持主题切换、深色模式" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "搜索功能：支持文章标题、内容搜索" })]
        }),
        new Paragraph({
          heading: HeadingLevel.HEADING_3,
        ...
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "用户管理：支持注册、登录、权限管理" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "文章管理：支持创建、编辑、发布、删除" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "评论管理：支持审核、删除" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "标签管理：支持创建、编辑、删除" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "分类管理：支持创建、编辑、删除" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "网站设置：支持基本信息配置" })]
        }),
        new Paragraph({
          heading: HeadingLevel.HEADING_2,
        ...
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "性能：页面加载时间不超过3秒" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "安全性：采用JWT认证，防止未授权访问" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "可靠性：具有良好的容错能力，确保服务稳定运行" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "可扩展性：采用模块化设计，便于后续功能扩展" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
          children: [new TextRun({ text: "可维护性：代码具有良好的可读性和可维护性" })]
        }),
        ...
        new Paragraph({
          children: [new TextRun({
            text: "前端采用Vue 3 + TypeScript + Element Plus技术栈，使用组件化开发方式，将页面拆分为多个可复用组件。通过Axios与后端API通信，获取数据并展示。前端项目结构清晰，分为apis、components、views、stores、router、assets、utils等目录，方便代码管理和维护。"
          })
        ],
        spacing: { before: 360, after: 360 }
        }),
        ...
        new Paragraph({
          children: [new TextRun({ text: "后端采用Spring Boot + MyBatis Plus技术栈，采用分层架构设计：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "前端项目采用模块化设计，主要目录包括：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "文章展示：使用Vue组件实现文章列表、详情页，支持分页、分类、标签筛选" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "用户交互：实现注册、登录、评论、点赞、收藏功能，使用JWT进行身份认证" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "后端项目采用分层架构设计，主要目录包括：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "用户管理：实现注册、登录、权限管理，使用Spring Security和JWT进行身份认证和授权" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "文章管理：实现CRUD操作，支持Markdown格式的文章内容" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "评论管理：实现CRUD操作，支持评论审核功能" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "标签管理：实现CRUD操作，支持文章与标签的关联" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
          children: [new TextRun({ text: "分类管理：实现CRUD操作，支持文章的分类管理" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "根据设计的数据库表结构，使用MySQL创建相应的表，并实现表之间的关联关系。使用MyBatis Plus实现数据库操作，简化数据访问层的代码。以下是部分核心表的创建SQL：" })]
        }),
        ...
        new Paragraph({
          children: [new TextRun({ text: "对系统的各项功能进行测试，包括：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "对系统的性能进行测试，包括：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "对系统的安全性进行测试，包括：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "通过测试，系统各项功能均能正常运行，性能满足要求，安全性良好。具体测试结果如下：" })]
        }),
        new Paragraph({
          numbering: { reference: "bullets", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "我设计并实现的基于Spring Boot和Vue 3的博客系统具有以下特点：" })]
        }),
        new Paragraph({
          numbering: { reference: "numbers", level: 0 },
        ...
        new Paragraph({
          children: [new TextRun({ text: "我成功实现了一个功能完善、性能优异、用户体验良好的博客系统，展示了软件工程专业所学知识在实际项目中的应用。系统采用现代化的技术栈，具有良好的可维护性和可扩展性，为后续的功能扩展和优化奠定了基础。通过本项目的开发，我不仅巩固了所学的专业知识，还提高了实际开发能力和解决问题的能力，为未来的职业发展打下了坚实的基础。" })]
        }),
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("e:\\Github2\\毕业设计\\new-blog\\blog_documents\\基于Spring Boot和Vue 3的博客系统设计与实现_毕业论文_钟奇林_humanized.docx", buffer);
  console.log("Humanized论文生成完成！");
});