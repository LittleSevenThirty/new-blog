...
import { cancelLike, userLike } from '../../apis/like';
import { ElMessage, ElMessageBox } from 'element-plus';
import { cancelFavorite, userFavorite } from '../../apis/favorite';
...
import { addArticleVisit, getArticleDetail } from '../../apis/article/index.ts';
import { ARTICLE_VISIT_PREFIX } from '../../const/visit.ts';
import { isLike } from '../../apis/like';
import { isFavorite } from '../../apis/favorite';
import { throttle } from '../../utils/tools.ts';
...
function likeBtn(detail: Detail) {
  if (like.value) {
    // 取消点赞
    cancelLike(1, detail.articleId).then((res: any) => {
      if (res.code === 200) {
        detail.likeCount -= 1
        like.value = false
    ...
    // 点赞
    userLike(1, detail.articleId).then((res: any) => {
      if (res.code === 200) {
        detail.likeCount += 1
        like.value = true
...
function collectionBtn(detail: Detail) {
  if (collection.value) {
    // 取消收藏
    cancelFavorite(1, detail.articleId).then((res: any) => {
      if (res.code === 200) {
    ...
    // 收藏
    userFavorite(1, detail.articleId).then((res: any) => {
      if (res.code === 200) {
        detail.favoriteCount += 1
...
async function getArticleDetailById() {
  getArticleDetail(route.params.id.toString()).then((res: any) => {
    if (!res.data) {
      ElMessage.warning({
        message: '文章不存在',
      })
...
function isFavoriteFunc() {
  isFavorite(1, articleDetail.value?.articleId as string).then((res: any) => {
...
// 是否点赞
function isLikeFunc() {
  isLike(1, articleDetail.value?.articleId).then((res: any) => {
        ...
        <template #content>
          ...
          <div class="p-1">
            ...
            <div>
              <!-- 富文本预览 -->
              <div>
                <MdPreview :editorId="id" :theme="mode as Themes" :modelValue="articleDetail?.articleContent"
              ...
              <div class="copyright">
                ...
                <div class="link">
                  ...
                  <strong>本文链接： </strong>
                  <a class="copyright_a" :href="env.VITE_FRONTEND_URL + $route.path">{{ env.VITE_FRONTEND_URL +
                    $route.path }}</a>
                </div>
                <div class="license">
                  <div>
            ...
            <div style="display: flex;justify-content: space-between">
              <div class="tag">
                <template v-for="tag in articleDetail?.tags" :key="tag.tagId">
                  <div @click="$router.push(`/tags/${tag.tagId}`)"># {{ tag.tagName }}</div>
                </template>
              </div>
              <div class="like">
                <div @click="likeBtn(articleDetail as Detail)">
                  <SvgIcon v-show="!like" name="like" />
                  <SvgIcon v-show="like" name="like-selected" />
                  <span>{{ articleDetail?.likeCount }}</span>
                </div>
                <div @click="collectionBtn(articleDetail as Detail)">
              ...
              <el-tooltip class="box-item" effect="light" placement="top">
                <template #content>
                  <div class="qrCode">
                    <div>
                      支付宝
                      <el-image :src="env.ZHI_FU_BAO_IMG" />
                    </div>
                  </div>
                </template>
                <div>
            ...
            <div class="goOn">
              <!-- 上一篇 -->
              <div>
                <div v-if="articleDetail?.preArticleId as number > 0">
              ...
              <!-- 下一篇 -->
              <div>
                <div v-if="articleDetail?.nextArticleId as number > 0">
            ...
            <!-- 用户评论 -->
            <Comment :type="1" :like-type="2" :author-id="Number(articleDetail?.userId)"
              :type-id="Number(articleDetail?.articleId)" v-if="loading" />
          </div>
        </template>
        <template #information>
          ...
          <div class="sticky_layout">
            ...
            <div v-if="articleDetail?.categoryId !== ''">
              <RandomArticle :categoryId="articleDetail?.categoryId.toString()"
                :articleId="route.params.id as string | undefined" title="相关推荐" prefix-icon="query_tasks" />
            </div>
          </div>
        </template>
        <template #footer>
        ...
        <div>
          ...
          <div class="copyright">
            ...
            <div class="link">
              <svg-icon name="author_link"></svg-icon>
              <strong>本文链接： </strong>
              <a class="copyright_a" :href="env.VITE_FRONTEND_URL + $route.path">{{ env.VITE_FRONTEND_URL + $route.path
              }}</a>
            </div>
            <div class="license">
              <div>
