...
const props = defineProps({
  ...
  direction: {
    type: String,
    default: 'level',
    validator: (value: string) => ['level', 'vertical'].includes(value)
  }
});
...
const calculateOffset = () => {
  ...
  if (elementInView) {
    ...
    // 应用速度和方向
    const direction = props.direction === 'vertical' ? -1 : 1;
...
<template>
  <div ref="parallelContainer" class="paraller" :style="{transform:`translate(${offset}px)`,transition:'transition 0.3s ease-out'}">
...
.paraller{
  will-change:transform;  // 告知浏览器可能会发生变化的属性
}
</style>