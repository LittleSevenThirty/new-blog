...
import { addComment } from "../../apis/comment/index.ts";
...
// 防止事件传播和默认行为
function preventDefaultAndKeepFocus(event: any) {
  // 阻止事件冒泡
  event.stopPropagation();
...
<template>
  <transition name="el-zoom-in-top">
    <div class="reply" v-show="comment.showReplyBox">
      <textarea style="color: #7B5F69;" ref="myInput" v-model="comment.replyText"
        :placeholder="'@' + comment.commentUserNickname" />
      <div class="reply-footer">
        <div class="reply-tools">
          <EmojiPicker ref="emojiPopover" :popover-width="510" @select-emoji="handleEmojiSelect"
            @operation-complete="handleOperationComplete" @mousedown.stop.prevent @click.stop.prevent>
            <template #trigger>
              <div class="emoji-trigger-btn" @click.stop.prevent="handleEmojiButtonClick" @mousedown.stop.prevent>
