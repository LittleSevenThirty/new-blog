...
import { userLike, cancelLike, isLike } from "../../apis/like/index.ts";
...
function getComments(typeId: number, pageNum: string, pageSize: string) {
  getComment(<number>props.type, typeId, pageNum, pageSize).then((res: any) => {
    console.log(res);
    if (res.code == 200) {
...
// 点赞
function likeBtn(comment: Comment) {
  userLike(<number>props.likeType, comment.commentId.toString()).then((res: any) => {
    if (res.code === 200) {
      comment.isLike = true
      comment.likeCount += 1
...
// 取消点赞
function cancelLikeBtn(comment: Comment) {
  cancelLike(<number>props.likeType, comment.commentId.toString()).then((res: any) => {
    if (res.code === 200) {
...
// 修改 replyBtn 函数
function replyBtn(comment: Comment[], id: number) {
  // 在对应评论项目显示回复框
...
function isLikeFunc() {
  isLike(<number>props.likeType).then((res: any) => {
    if (res.code === 200) {
      res.data.forEach((item: any) => {
        comments.value.forEach((comment: any) => {
          if (comment.commentId === item.typeId) {
            comment.isLike = true
          }
          // 递归子评论
          if (comment.childComment && comment.childComment.length) {
...
function recursionChildComment(childComment: any, res: any) {
  res.data.forEach((item: any) => {
    childComment.forEach((child: any) => {
      if (child.commentId === item.typeId) {
        child.isLike = true
      }
      if (child.childComment && child.childComment.length) {
        ...
        <div class="comment_content">
          <div class="comment_content_header">
            ...
            <div>
              ...
              <svg-icon @click="replyBtn(comments, comment.commentId)" name="comment"
          ...
          <template v-if="comment.childComment && comment.childComment.length">
            <ChildComment :reply-btn="replyBtn" :like-btn="likeBtn" :cancel-like-btn="cancelLikeBtn" :comment="comment"
              :author-id="authorId" :show-all-child-comments="showAllChildComments" :get-comments="getComments"
              :page-size="pageSize" :type="type" :active-comment-id="activeCommentId"
