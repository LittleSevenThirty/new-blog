package cn.edu.tjufe.zql.intercepter;
 ...
 *
 * @authro 钟奇林
 * @description Redis限流注解拦截器-发生在spring security安全验证过滤链之后
 * @date 2026/3/14
...
public class AccessLimitIntercepter implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(handler instanceof HandlerMethod handlerMethod){
            ...
            String method=request.getMethod();  // 获取get还是post还是。。
            String uri=request.getRequestURI();
            String key="limit:"+method+":"+uri+":"+ip;

            try{
                // 设置过期时间key
                String expireTimeKey=EXPIRE_TIME+":"+key;
                // 尝试key值自增
                ...
                // 超出访问限制
                if (count > maxCount) {
                    WebUtil.renderString(response, ResponseResult.failure(ResponseEnum.REQUEST_FREQUENTLY.getCode(), accessLimit.message()).asJsonString());
                    // 限制
...
//                    blackListMapper.delete(new LambdaQueryWrapper<BlackList>().eq(BlackList::getUserId, SecurityUtils.getUserId()));
                ...
                WebUtil.renderString(response, ResponseResult.failure(ResponseEnum.BLACK_LIST_ERROR.getCode(), StrUtil.format("已被封禁，无法访问，距解封剩余：{}", DateUtil.formatBetween(new Date(), date, BetweenFormatter.Level.SECOND))).asJsonString());
                return true;
            }
        }
        return false;
...
//            WebUtil.renderString(response, ResponseResult.failure(ResponseEnum.BLACK_LIST_ERROR.getCode(),
